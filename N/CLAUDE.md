...
### VHDL Compilation
```bash
# Compile all VHDL source files with GHDL
make compile
...
# Run P1 (BASIC) tests - minimal output
python run.py
...
### Hardware Tests on Real Moku Device
```bash
# Navigate to hardware test directory
cd tests/hw

# Run P1 basic hardware tests
...
# Specify custom slots
python run_hw_tests.py 192.168.8.98 --bitstream ../DPD-bits.tar --osc-slot 1 --cc-slot 2
```

### Python Utilities Testing
```bash
# Test clock conversion utilities
python py_tools/clk_utils.py

# Test DPD configuration dataclass
...
Configuration registers (CR2-CR10) are only propagated when the FSM is in INITIALIZING state. This prevents race conditions from asynchronous network register updates. See `N/network-register-sync.md` for details.
...
├── N/                                  # Design notes (Obsidian-compatible)
│   ├── CLAUDE.md                       # This file
│   ├── HVS.md                          # HVS encoding documentation
│   ├── network-register-sync.md        # Sync protocol documentation
│   ├── hardware_debug_checklist.md     # Debugging guide
│   └── dpd_utils_README.md             # Python utilities documentation
